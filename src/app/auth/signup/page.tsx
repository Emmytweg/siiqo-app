...
const Signup = () => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      /* // --- LIVE API CODE (Commented out for later use) ---
      const requestBody: any = {
        country: formData.country,
        email: formData.email,
        fullname: formData.name,
      ...
      */
      ...
      // --- MOCK TESTING CODE (Currently Active) ---
      await new Promise((resolve) => setTimeout(resolve, 1500));
      const data: SignupResponse = DUMMY_SIGNUP_SUCCESS;
      // --------------------------------------------
      ...
      if (data && (data.status === "success" || (data as any).success)) {
        ...
        const userDataToSave = {
          ...
          role: "shopper", // Assigned shopper role by default
        ...
        setTimeout(() => {
          showNotification(
            "success",
            "Account Created Successfully!",
            "TEST MODE: Details saved. Use any 6-digit code to verify."
          );
          ...
          setTimeout(() => {
            router.push(`/auth/verify-otp?email=${encodeURIComponent(userDataToSave.email)}`);
          }, 2000);
        }, 500);
      } else {
        throw new Error(data.message || data.error || "Signup failed");
    ...
    <>
      ...
      <div className="flex items-center justify-center min-h-screen p-4 py-10 bg-gradient-to-br  from-blue-50 via-white to-green-50">
        <div className="w-full max-w-2xl">
          ...
          <Card className="border min-h-screen border-gray-200 shadow-xl bg-white/80 backdrop-blur-sm">
            <CardHeader className="pb-2 text-center">
              <div className="flex items-center justify-center w-12 h-12 mx-auto mb-4 bg-gradient-to-br from-blue-500 to-purple-600 rounded-xl">
                <Sparkles className="w-6 h-6 text-white" />
              </div>
