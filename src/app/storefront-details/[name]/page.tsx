...
import { ArrowLeft, MapPin, Clock, Globe, MessageCircle, Star, ShoppingBag, Heart } from "lucide-react";
    ...
    const fetchCatalogs = async () => {
      try {
        ...
        if (res.status === "success" && res.catalogs) {
          // Keep catalogs as-is for "View by Catalogs" mode
          // For "View by Products" mode, we'll flatten them
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold">{store.branding.layout_style}</p>
                  </div>
                </div>
              ...
              selectedCatalog ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="border rounded-2xl bg-white shadow-sm overflow-hidden">
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          {selectedCatalog.description && (
                            <p className="text-sm text-slate-600 mt-2">{selectedCatalog.description}</p>
                          )}
                    ...
                    {/* Products Grid */}
                    {selectedCatalog.products && selectedCatalog.products.length > 0 ? (
                      <div className="p-6">
                          ...
                          {selectedCatalog.products.map((product: any) => (
                            <div key={product.id} className="group cursor-pointer">
                              <div className="relative aspect-square overflow-hidden rounded-xl bg-slate-100 border border-slate-100 shadow-sm transition-all group-hover:shadow-md">
                                <Image 
                                  src={product.image || "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?auto=format&fit=crop&q=80&w=400"} 
                                  alt={product.name} 
                                  fill
                              ...
                              <div className="mt-3 px-1">
                                <h4 className="text-[13px] font-bold text-slate-800 line-clamp-2">{product.name}</h4>
                ...
                catalogs.filter((c: any) => c.name).length > 0 ? (
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                      {catalogs
                        .filter((c: any) => c.name)
                        .slice((catalogPage - 1) * catalogsPerPage, catalogPage * catalogsPerPage)
                        .map((catalog: any) => (
                          <div
                            key={catalog.id}
                          ...
                          >
                            ...
                            <div className="p-4">
                              ...
                              {catalog.description && (
                                <p className="text-xs text-slate-600 mt-2 line-clamp-2">{catalog.description}</p>
                              )}
                    ...
                    {/* Pagination */}
                    {catalogs.filter((c: any) => c.name).length > catalogsPerPage && (
                      <div className="flex items-center justify-center gap-2 mt-8 pt-6 border-t">
                        ...
                        <div className="flex gap-1">
                          {Array.from({ length: Math.ceil(catalogs.filter((c: any) => c.name).length / catalogsPerPage) }).map((_, i) => (
                            <button
                              key={i + 1}
                        ...
                        <button
                          ...
                          disabled={catalogPage >= Math.ceil(catalogs.filter((c: any) => c.name).length / catalogsPerPage)}
                          className="px-4 py-2 border rounded-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50 transition-colors"
                        >
                          Next
                        </button>
                  ...
                  <div className="border rounded-2xl bg-white shadow-sm p-8 text-center">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 mb-2">No Catalogs Yet</h2>
                    <p className="text-gray-600 mb-6">
                      This store hasn't created any catalogs yet. Check back later!
                    </p>
                  </div>
                )
              )
              ...
              (() => {
                ...
                return allProducts.length > 0 ? (
                    ...
                    <div className="border rounded-2xl bg-white shadow-sm p-6">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                        {allProducts.map((product: any) => (
                          <div key={`${product.catalogName}-${product.id}`} className="group cursor-pointer">
                            <div className="relative aspect-square overflow-hidden rounded-xl bg-slate-100 border border-slate-100 shadow-sm transition-all group-hover:shadow-md">
                              <Image 
                                src={product.image || "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?auto=format&fit=crop&q=80&w=400"} 
                                alt={product.name} 
                  ...
                  <div className="border rounded-2xl bg-white shadow-sm p-8 text-center">
                    ...
                    <h2 className="text-2xl font-bold text-gray-900 mb-2">Products Coming Soon</h2>
                    <p className="text-gray-600 mb-6">
                      This store is setting up their product catalog. Check back later!
                    </p>
                    <button
