...
import { 
  ChevronLeft, Camera, Package, Check, 
  Search, Loader2, Plus, Info, ShoppingBag, FolderPlus 
} from "lucide-react";
...
const AddCatalog = () => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (res.status === "success") {
          // Extract products from the 'Standalone Products' group provided in your response
          const standalone = res.data.find((item: any) => item.catalog_name === "Standalone Products");
          setAvailableProducts(standalone?.products || []);
        }
      } catch (error) {
    ...
    const fetchCatalogs = async () => {
      try {
        ...
        if (res.status === "success" && res.catalogs) {
          const catalogs = res.catalogs.filter((cat: any) => cat.name !== "Standalone Products");
          setExistingCatalogs(
  ...
  const handleCreateCatalog = async () => {
    if (mode === 'new' && !catalogName.trim()) {
      toast({ title: "Name Required", description: "Please enter a catalog name" });
      return;
    }

    if (mode === 'existing' && !selectedCatalogId) {
      toast({ title: "Selection Required", description: "Please select a catalog" });
      return;
    }

    if (selectedProductIds.length === 0) {
      toast({ title: "No Products", description: "Please select at least one product" });
      return;
    }
    ...
    try {
      ...
      if (mode === 'new') {
        ...
        const targetCatalog = existingCatalogs.find((c) => c.id === selectedCatalogId);
      ...
      if (data.status === "success") {
        toast({ title: "Success", description: data.message || `Catalog ${mode === 'new' ? 'created' : 'updated'} successfully!` });
    ...
    <div className="min-h-screen bg-[#F0F2F5] font-sans pb-24">
      ...
      <div className="bg-[#008069] text-white px-4 py-4 flex items-center justify-between sticky top-0 z-50 shadow-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-lg font-medium">New collection</h1>
      ...
      <div className="max-w-xl mx-auto p-4 space-y-4">
        ...
        <div className="bg-white rounded-lg shadow-sm p-4 space-y-4">
          <div className="flex flex-col items-center py-4 border-b border-slate-100">
            <div className="w-24 h-24 bg-slate-100 rounded-full flex items-center justify-center text-[#008069] mb-2 border-2 border-dashed border-slate-300 overflow-hidden">
              {catalogImagePreview ? (
                <img src={catalogImagePreview} alt="Catalog" className="w-full h-full object-cover" />
              ) : (
                <Camera size={32} />
              )}
            </div>
            <label className="text-xs font-bold text-[#008069] uppercase cursor-pointer" htmlFor="catalog-image-input">
          ...
          <div className="space-y-4">
            <div className="border-b-2 border-slate-100 focus-within:border-[#008069] transition-all">
              <label className="text-[10px] font-bold text-[#008069] uppercase">Collection Name</label>
              <input 
                type="text" 
            ...
            <div className="border-b-2 border-slate-100 focus-within:border-[#008069] transition-all">
              <label className="text-[10px] font-bold text-[#008069] uppercase">Description (Optional)</label>
              <textarea 
        ...
        {mode === 'existing' && (
          <div className="bg-white rounded-lg shadow-sm">
            <div className="p-4 border-b border-slate-100">
              <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider">Select Collection</h3>
            </div>

            {fetchingCatalogs ? (
              <div className="p-10 flex flex-col items-center justify-center text-slate-400">
                <Loader2 className="animate-spin mb-2" />
                <p className="text-xs font-bold uppercase">Loading collections...</p>
              </div>
                ...
                {existingCatalogs.map((catalog) => (
                  ...
                  >
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                      ...
                      {selectedCatalogId === catalog.id && <Check size={12} className="text-white" />}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-bold text-slate-900 truncate">{catalog.name}</p>
              ...
              <div className="p-10 flex flex-col items-center justify-center text-slate-400">
                ...
                <p className="text-xs font-bold uppercase">No collections yet</p>
                <button
                  onClick={() => setMode('new')}
        ...
        <div className="bg-white rounded-lg shadow-sm">
          <div className="p-4 border-b border-slate-100 flex justify-between items-center">
            <h3 className="text-sm font-bold text-slate-500 uppercase tracking-wider">Select Products</h3>
          ...
          {fetchingProducts ? (
            <div className="p-10 flex flex-col items-center justify-center text-slate-400">
              <Loader2 className="animate-spin mb-2" />
              <p className="text-xs font-bold uppercase">Loading your store...</p>
            </div>
          ) : (
              ...
              { availableProducts.length > 0 ? availableProducts.map((product) => (
                ...
                >
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                    ...
                    {selectedProductIds.includes(product.id) && <Check size={12} className="text-white" />}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-slate-400 font-medium">{product.category}</p>
                  </div>

                  <div className="text-right">
                    <p className="text-sm font-black text-slate-900">₦{product.final_price?.toLocaleString()}</p>
                  </div>
                </div>
              )) : (
                <div className="p-10 flex flex-col items-center justify-center text-slate-400">
                  <Package size={32} className="mb-2" />
                  <p className="text-xs font-bold uppercase">No products found in your store. </p>
                  <Link href="/vendor/products" className="text-sm font-bold text-[#008069] mt-2"> Add Products </Link>
                </div>
              )}
            </div>
          )}
        ...
        <div className="flex items-start gap-3 p-4 bg-[#D1E9FF] rounded-lg text-[#004E96]">
          ...
          <p className="text-[11px] font-medium leading-relaxed">
            Creating a collection helps customers find your products faster by grouping related items together.
          </p>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-100 flex justify-center">
        <button 
          onClick={handleCreateCatalog}
          disabled={loading || selectedProductIds.length === 0 || (mode === 'existing' && !selectedCatalogId)}
          className={`w-full max-w-md py-4 rounded-full text-white font-black text-xs uppercase tracking-widest shadow-lg transition-all active:scale-95 ${
            selectedProductIds.length > 0 && (mode === 'new' || selectedCatalogId) 
              ? "bg-[#1E293B]" 
        ...
        >
          {loading 
            ? "Processing..." 
            : mode === 'new' 
              ? `Create Collection (${selectedProductIds.length} Items)` 
              : `Add to Collection (${selectedProductIds.length} Items)`
          }
        </button>
      </div>
    </div>
