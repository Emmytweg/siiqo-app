...
getPublicCatalogs: async () => {
  ...
  createCatalog: async (catalogData: { name: string; description?: string; product_ids: number[]; image?: File | null }) => {
  ...
  updateCatalog: async (
    catalogId: number,
    catalogData: { name?: string; description?: string; product_ids?: number[]; image?: File | null }
  ) => {
    const formData = new FormData();
    ...
    const response = await api.patch(`/products/catalogs/${catalogId}`, formData, {
      headers: { "Content-Type": "multipart/form-data" },
    });
    return response.data;
  },
